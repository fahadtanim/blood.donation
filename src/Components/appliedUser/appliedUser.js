    ...
    handleTablePrint = (param) =>{
        ...
        var donorList = data.map(function(data,index){
            ...
            <tr key = {index}>
                ...
                <td>{data.email}</td>
                <td>{data.org_name}</td>
                <td><button className = "waves-effect waves-light btn-large" onClick={() => axios.put("http://139.59.91.220:8080/bloodbank/api/bloodBank/v1/exhort/approve/"+data.user_id).then(result => {
          console.log(result);
      })}>Approve</button></td>
            </tr>);
          })
